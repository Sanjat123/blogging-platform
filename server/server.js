...
server.post("/github-auth", async (req, res) => {
    ...
    if (!code) {
        return res.status(403).json({ "error": "Failed to get auth code from GitHub" });
    }
    ...
    // 2. Use access token to get user's data
    let githubUser, githubEmail;
    ...
    try {
        ...
        githubEmail = emailResponse.data.find(email => email.primary && email.verified).email;
        
        if (!githubEmail) {
            return res.status(403).json({ "error": "Please verify your primary email on GitHub to sign in." });
        }

    } catch (err) {
        console.log(err.message);
        return res.status(500).json({ "error": "Failed to get user data from GitHub" });
    }

    // 3. Find or create user in your database (same logic as Google Auth)
    let { name, login, avatar_url } = githubUser;
    let email = githubEmail;

    let user = await User.findOne({"personal_info.email": email}).select("personal_info.fullname personal_info.username personal_info.profile_img google_auth");

    if (user) {
        if (user.google_auth) { // Optional: Check if they signed up with Google
            return res.status(403).json({ "error": "This email was signed up with Google. Please log in with Google." });
        }
    } else {
        // Sign up new user
        let username = await generateUsername(login); // Use GitHub login to generate username

        user = new User({
            personal_info: { 
                fullname: name || login, // Use login as fallback for name
                email: email, 
                username: username,
                profile_img: avatar_url
            },
            google_auth: false // You might want to rename this field to `social_auth`
        });
